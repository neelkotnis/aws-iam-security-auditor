...
-------------
Checks:
  KMS-001  CMK rotation not enabled                     -> MEDIUM (CIS 3.8)
  KMS-002  Key policy allows Principal: *               -> CRITICAL
  KMS-003  Key pending deletion                         -> HIGH
  KMS-004  Key grant to external principal              -> HIGH
"""
...
def _customer_keys(kms):
    try:
        ...
        logger.error("KMS: list_keys failed: %s", e)
        return []


def check_key_rotation(kms, account_id, region):
    findings = []
    for key in _customer_keys(kms):
        kid = key["KeyId"]
        try:
            ...
            if not resp.get("KeyRotationEnabled", False):
                findings.append(Finding(
                    check_id="KMS-001",
                    check_name="KMS CMK Rotation Not Enabled",
                    severity=Severity.MEDIUM,
                    resource=key.get("Arn", kid),
                    account_id=account_id,
                    region=region,
                    detail=f"KMS key '{kid}' does not have automatic annual rotation enabled.",
                    remediation=f"aws kms enable-key-rotation --key-id {kid}",
                    cis_control=get_cis("KMS-001"),
                    compliance_controls=get_controls("KMS-001"),
                ))
        except Exception as e:
        ...
        try:
            ...
            for stmt in policy.get("Statement", []):
                ...
                if is_star:
                    findings.append(Finding(
                        check_id="KMS-002",
                        check_name="KMS Key Policy Allows Principal: *",
                        severity=Severity.CRITICAL,
                        resource=key.get("Arn", kid),
                        account_id=account_id,
                        region=region,
                        detail=f"KMS key '{kid}' has a policy with Principal: '*' — any AWS principal can use it.",
                        remediation="Replace Principal: '*' with specific IAM role or account ARNs.",
                        cis_control=get_cis("KMS-002"),
                        compliance_controls=get_controls("KMS-002"),
                    ))
                    break
        except Exception as e:
            ...
            for k in page.get("Keys", []):
                try:
                    ...
                    if meta.get("KeyState") == "PendingDeletion":
                        findings.append(Finding(
                            check_id="KMS-003",
                            check_name="KMS Key Pending Deletion",
                            severity=Severity.HIGH,
                            resource=meta.get("Arn", k["KeyId"]),
                            account_id=account_id,
                            region=region,
                            detail=(
                                f"KMS key '{k['KeyId']}' is scheduled for deletion on "
                                f"{meta.get('DeletionDate', 'unknown')}. "
                                "Data encrypted with this key will become inaccessible."
                            ),
                            remediation=f"aws kms cancel-key-deletion --key-id {k['KeyId']} if accidental.",
                            cis_control=get_cis("KMS-003"),
                            compliance_controls=get_controls("KMS-003"),
                        ))
                except Exception:
                    pass
    except Exception as e:
            ...
            for page in paginator.paginate(KeyId=kid):
                for grant in page.get("Grants", []):
                    ...
                    if grantee and account_id not in grantee and "amazonaws.com" not in grantee:
                        findings.append(Finding(
                            check_id="KMS-004",
                            check_name="KMS Key Grant to External Principal",
                            severity=Severity.HIGH,
                            resource=key.get("Arn", kid),
                            account_id=account_id,
                            region=region,
                            detail=(
                                f"KMS key '{kid}' has a grant to external principal '{grantee}'. "
                                "Third parties have active cryptographic access to your key."
                            ),
                            remediation=(
                                f"aws kms retire-grant --key-id {kid} "
                                f"--grant-id {grant.get('GrantId')} if not expected."
                            ),
                            cis_control=get_cis("KMS-004"),
                            compliance_controls=get_controls("KMS-004"),
                        ))
        except Exception as e:
...
def run(session: boto3.Session) -> list[Finding]:
    ...
    findings = []
    findings.extend(check_key_rotation(kms, aid, region))
    findings.extend(check_key_policy(kms, aid, region))
    findings.extend(check_keys_pending_deletion(kms, aid, region))
    findings.extend(check_key_grants(kms, aid, region))
    return findings
